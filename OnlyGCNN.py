...
sys.path.insert(0, '..')
from lib import models, graph, coarsening, utilsNo
...
# '''
'''
## Code with same behavior than original to check the data manipulation
allData= np.transpose(np.hstack((D1,D2,D3,D4,D5)))
allLabel = (np.vstack((L1,L2,L3,L4,L5)))
indexes= np.random.choice(np.arange(allData.shape[0]), int(allData.shape[0]*0.3), replace=False)
indexes.sort()
print(indexes)
print(allData.shape)
Test_Data=[]
...
model_perf = utilsNo.model_perf()#utilsNo.model_perf()
...
params['brelu'] = 'b1relu'
## To compute gene perturbation replace import utilsNO by utils
