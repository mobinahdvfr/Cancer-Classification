...
class model_perf(object):
    ...
    def test(s, model, name, params, train_data, train_labels, val_data, val_labels, test_data, test_labels):
        ...
        s.fit_accuracies[name], s.fit_losses[name], s.fit_time[name], s.train_accuracies[name], s.train_losses[name], s.epoch[name]= \
        ...
        print('test  {}'.format(string))
        alldata=np.append(train_data, val_data, axis= 0)
        #alldata=np.append(alldata, test_data, axis= 0)
        alllabels= np.append(train_labels, val_labels, axis= 0)
        #alllabels= np.append(alllabels, test_labels, axis=0)
        ## Modified version with all data to get the entire confusion matrix
        string, s.val_accuracy[name], s.val_f1[name], s.val_loss[name] =  model.evaluate(alldata, alllabels,con = True, Name = 'Val')
        ## Original version with test data
        #string, s.val_accuracy[name], s.val_f1[name], s.val_loss[name] = model.evaluate(val_data, val_labels,con = True, Name = 'Val')
        ...
        '''
        print(train_data.shape) -> PPIS(8850, 7446)
        print(test_data.shape)  -> PPIS(2220, 7446)

        '''
        Knockdown = []
        Knockup = []
        ## Append the original labels
        # Knockdown.append(test_labels)# @@@@@@@@@@@@@@@@@@@@@@@ treure @@@@@@@@@@@@@@@@@@@
        # Knockup.append(test_labels)
        ## 7100 is hardcoded. In PPIS there are 7447 genes in each sample, not 7100
        ...
        #for i in range(3200, test_data.shape[1]):
            ...
            # if i%1000==0 and i!=0:
            #     with open("Datadown"+str(i//1000)+".csv", "wb") as f:
            #         np.savetxt(f, np.array(Knockdown).astype(int), fmt='%i', delimiter=",", newline='\n')
            #         f.close()
            #     with open("Dataup"+str(i//1000)+".csv", "wb") as f:
            #         np.savetxt(f, np.array(Knockup).astype(int), fmt='%i', delimiter=",", newline='\n')
            #         f.close()
            #     Knockup=[]
            #     Knockdown=[]
            #     import time
            ...
            Datadown = test_data
            Datadown[:,i]=0
            Knockdown.append(np.array(model.evaluate(Datadown, test_labels, pre = True)).astype(np.int8))
            Dataup = test_data
            Dataup[:,i]=1
            Knockup.append(np.array(model.evaluate(Dataup, test_labels, pre = True)).astype(np.int8))
            #print(' On gene number \t{} upzeros {}, downZeros {}'.format(i, Knockup[-1].size - np.count_nonzero(Knockup[-1]), Knockdown[-1].size - np.count_nonzero(Knockdown[-1])), end="\r")
            print(' On gene number \t{} '.format(i), end="\r")
        Knockdown=np.array(Knockdown).astype(np.int8)
        Knockup=np.array(Knockup).astype(np.int8)
        with open("DatadownN.csv", "wb") as f:
            np.savetxt(f, (Knockdown).astype(int), fmt='%i', delimiter=",", newline='\n')
            f.close()
        with open("DataupN.csv", "wb") as f:
            np.savetxt(f, (Knockup).astype(int), fmt='%i', delimiter=",", newline='\n')
            f.close()
