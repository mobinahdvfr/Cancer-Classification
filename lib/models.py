...
class base_model(object):
    ...
    def predict(self, data, labels=None, sess=None):
        ...
        if labels is not None:
            return predictions, loss * self.batch_size / size ## TODO check resultat
        else:
            return predictions
    ...
    def evaluate(self, data, labels, sess=None, con = False, Name = None, pre=False): # test = TRUE
        ...
        if con is True:
            ...
            '''
            if Name is 'Test': name= 'Confusion_Test_Pred'
            elif Name is 'Train': name= 'Confusion_Train'
            elif Name is 'Val': name= 'Confusion_Val'
            cfmat=met.confusion_matrix(labels, predictions).astype(int)
            fig, ax = plt.subplots()
            im = ax.imshow(cfmat, aspect='auto', interpolation='nearest', cmap='GnBu') #cmap
            for i in range(len(tumours)):
                for j in range(len(tumours)): 
                    if j!=i: ax.text(j, i, str(cfmat[i, j]), ha="center", va="center", color="b", size= 'xx-small')
                    else: 
                        if cfmat[i,j]<900:ax.text(j, i, str(cfmat[i, j]), ha="center", va="center", color="darkred", size= 'xx-small')
                        else: ax.text(j, i, str(cfmat[i, j]), ha="center", va="center", color="r", size= 'xx-small')
            ...
            with open(name+'.csv', "wb") as f:
                np.savetxt(f, cfmat, fmt='%i', delimiter=",", newline='\n')
                f.close()
            ## TODO exit aqui
...
class cgcnn(base_model):
    ...
    def __init__(self, L, F, K, p, M, filter='chebyshev5', brelu='b1relu', pool='mpool1',
                ...
                regularization=0, dropout=0, batch_size=200, eval_frequency=200,
                dir_name=''):
        super().__init__()
